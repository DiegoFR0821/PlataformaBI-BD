...
# ==========================================
# CAPA 3: ETL BLINDADO (Estandariza Mayúsculas)
# ==========================================
...
def ejecutar_etl():
    try:
        ...
        if not archivos:
            return jsonify({"estado": "ERROR", "mensaje": "No hay archivos CSV."})
        ...
        for nombre_archivo in archivos:
            ...
            for index, fila in df.iterrows():
                # Forzamos que se guarden estrictamente como 'TRUE' o 'FALSE' en texto plano
                uso_arma_limpio = str(fila['Uso_Arma']).upper().strip()
                captura_limpia = str(fila['Captura']).upper().strip()
                
                cursor.execute("""
                    INSERT INTO Staging_Reportes_Policiales 
                    (Fecha_Reporte, Hora_Reporte, Distrito, Tipo_Delito, Latitud, Longitud, Tiempo_Respuesta_Min, Uso_Arma, Captura, Rango_Horario, Estado_Limpieza)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Procesado')
                """, (
                    str(fila['Fecha_Reporte']), str(fila['Hora_Reporte']), str(fila['Distrito']), 
                    str(fila['Tipo_Delito']), float(fila['Latitud']), float(fila['Longitud']),
                    float(fila['Tiempo_Respuesta_Min']), uso_arma_limpio, captura_limpia, str(fila['Rango_Horario'])
                ))
        ...
        conexion.commit()
        conexion.close()
        return jsonify({"estado": "EXITO", "mensaje": f"ETL Completado: {registros_insertados} registros cargados."})
    except Exception as e:
        return jsonify({"estado": "ERROR", "mensaje": str(e)})
